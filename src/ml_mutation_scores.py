...
from process_evosuite_mutation_scores import ProcessEvoSuiteMutationScores

data = ProcessEvoSuiteMutationScores()
...
def get_significant_classes_matching_metrics(search_budget):
    ...
    # Contains the mutation score results per class from EvoSuite
    ...
    significant_classes_stats[str(key_budget + '_branch')] = data.get_significant_classes_stats(mutation_scores_filename, search_budget, 'branch')
    significant_classes_stats[str(key_budget + '_default')] = data.get_significant_classes_stats(mutation_scores_filename, search_budget, 'default')
    ...
    matching_classes_metrics[key_class_metrics] = data.get_matching_classes_metrics(mutation_scores_filename, search_budget)
...
def get_X_y(search_budget):
    ...
    medians = data.calculate_medians(search_budget)[['class', 'configuration', 'mutation_score_percent']]
    ...
    for cls in signif_matched_metrics[metrics_budget]['class'].items():
        ...
        if medians.loc[medians['class']==cls[1]].shape[0] != 3:
            continue
        X.append([])
...
def get_label(search_budget, class_name, medians, labels_dict=None):
    ...
    global how_many
    # Get matching rows
    selected_rows = medians.loc[medians['class']==class_name]
    ...
    max_coverage = selected_rows.max(numeric_only=True)['mutation_score_percent']
    ...
    max_rows = selected_rows.loc[selected_rows['mutation_score_percent']==max_coverage]
    ...
    max_config_id = max_rows.iloc[-1]['configuration']
